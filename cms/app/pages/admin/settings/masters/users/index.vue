        ...
        <v-breadcrumbs
          ...
          :items="[
            ...
            },
            {
              title: 'Users',
              disabled: true,
              to: `/admin/settings/masters/users`,
            },
          ]"
          divider="/"
        ></v-breadcrumbs>
                ...
                <v-row>
                  ...
                  >
                    <v-btn
                      ...
                      variant="outlined"
                      rounded="lg"
                      @click="downloadExcel('candidates')"
                    ...
                    <v-btn
                      ...
                      variant="elevated"
                      rounded="lg"
                      to="/admin/settings/masters/users/form"
                    >
                      Add Candidate
                    </v-btn>
                  </v-col>
              ...
              <template v-slot:item.actions="{ item }">
                <div class="d-flex align-center gap-2">
                  <v-btn
                    ...
                    color="primary"
                    density="compact"
                    :to="`/admin/settings/masters/users/form?id=${item.id}`"
                  ></v-btn>
                  <v-btn
                    icon="mdi-delete"
                    variant="text"
            ...
            >
              ...
              <template v-slot:item.id="{ item }">
                <v-btn
                  variant="text"
                  color="primary"
                  :to="`/admin/settings/masters/users/batch/${item.id}`"
                  class="text-none"
                >
                  {{ item.id }}
                </v-btn>
...
// Selected columns
const selectedColumns = useState("candidateSelectedColumns", () => [
  "index",
  "photo_url",
  "full_name",
  "email",
...
const visibleHeaders = computed(() => {
  if (!allHeaders.value || allHeaders.value.length === 0) {
    return [
      { title: "No", key: "index", sortable: false },
      { title: "Photo URL", key: "photo_url", sortable: true },
...
const viewBatchDetail = (batchId) => {
  ...
  navigateTo(`/admin/settings/masters/users/batch/${batchId}`);
};
...
const downloadExcel = async (tabType) => {
  downloadingExcel.value = true;
  try {
    // Build search filters based on tab type
    const searchFilters =
      tabType === "candidates"
        ? { ...filters.value }
        : { ...companyFilters.value };

    // Add search input to selected filter fields for candidates
    if (
      tabType === "candidates" &&
      searchInput.value &&
      filterFields.value.length > 0
    ) {
      const orWhere = [];
      filterFields.value.forEach((field) => {
        if (field) {
    ...
    // Add search input to selected filter fields for company users
    if (
      tabType === "companies" &&
      companySearchInput.value &&
      companyFilterFields.value.length > 0
    ) {
      const orWhere = [];
      companyFilterFields.value.forEach((field) => {
        if (field) {
    ...
    searchFilters.user_role =
      tabType === "candidates" ? "candidate" : "company";
    ...
    const response = await $apiFetchRaw("/users/search", {
      method: "GET",
      params: params,
    ...
    if (!validMimeTypes.includes(blob.type)) {
      ...
      a.href = url;
      const fileName =
        tabType === "candidates" ? "candidates.xlsx" : "company_users.xlsx";
      a.download = fileName;
      a.click();
