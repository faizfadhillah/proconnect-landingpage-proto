          ...
          >
            ...
            <v-row class="mt-0">
              ...
              >
                <v-list>
                  <template
                    v-for="(applicant, index) in jobs"
                    :key="applicant.job.id"
                  >
                    <v-list-item
                      @click="selectJob(applicant.job)"
                      class="pb-2 pt-1"
                      :class="{
                        'selected-job': selectedJob?.id === applicant.job.id,
                      }"
                    >
                        ...
                        <v-list-item-action>
                          ...
                          >
                            <v-img
                              ...
                              :src="BASE_URL + applicant.job.company?.logo_url"
                              cover
                            ></v-img>
                      ...
                      <v-list-item-title
                        ...
                        >{{ applicant.job.title }}</v-list-item-title
                      >
                      <div>
                        <div>
                          <span class="text-caption text-primary">
                            {{ applicant.job.company?.brand_name }}
                            <template v-if="applicant.job.company?.branch">
                              ({{ applicant.job.company?.branch }})
                            </template>
                          </span>
                          <template
                            v-if="
                              (applicant.job.is_outside_indo &&
                                applicant.job.other_region) ||
                              (!applicant.job.is_outside_indo &&
                                applicant.job.region)
                            "
                          >
                        ...
                        <div
                          ...
                          v-if="
                            applicant.job.min_salary || applicant.job.max_salary
                          "
                        >
                          {{
                            applicant.job.salary_country?.currency_code || "IDR"
                          }}
                          {{ formatNumber(applicant.job.min_salary) }} -
                          {{
                            applicant.job.salary_country?.currency_code || "IDR"
                          }}
                          {{ formatNumber(applicant.job.max_salary) }} per month
                        </div>
                        <div>
                          <span class="text-caption text-grey-darken-2">{{
                            formatTimeAgo(applicant.updated_at)
                          }}</span>
                          <span
                            class="text-caption text-red-darken-2 font-weight-bold"
                            v-if="applicant.job.status == 'CLOSE'"
                          >
                            - Closed
                          </span>
                        </div>
                        <div>
                          <template v-if="applicant.lastApplicantJobStep">
                            <template
                              v-if="
                                applicant.lastApplicantJobStep?.jobStep?.type ==
                                  'SYS_HIRED' &&
                                applicant.lastApplicantJobStep?.status ==
                                  'ACCEPTED'
                              "
                            >
                              <v-chip
                            ...
                            <template v-else>
                              <v-chip size="small" class="mr-1">{{
                                capitalizeWords(
                                  applicant.lastApplicantJobStep?.jobStep
                                    ?.step_name
                                )
                              }}</v-chip>
                              <v-chip
                          ...
                          <v-chip
                            v-else
                            size="small"
                            :color="getColorStatus(applicant.status)"
                            variant="tonal"
                          >
                            {{ capitalizeWords(applicant.status) }}
                          </v-chip>
                        </div>
                      </div>
                    </v-list-item>
              ...
              <v-list>
                <v-list-item class="pb-2">
                  ...
                  <v-list-item-title
                    ...
                    >{{ selectedJob.title }}</v-list-item-title
                  >
                  <div>
                    <div>
                      <span class="text-caption text-primary">
                    ...
                    <div>
                      ...
                      <span class="text-caption text-grey-darken-2">
                        ...
                        {{
                          formatDateShort(
                            jobs.find((a) => a.job.id === selectedJob.id)
                              ?.created_at
                          )
                        }}
                      </span>
                    ...
                    <div class="mt-2">
                      <template v-if="selectedApplicant.lastApplicantJobStep">
                        <template
                          v-if="
                            selectedApplicant.lastApplicantJobStep?.jobStep
                              ?.type == 'SYS_HIRED' &&
                            selectedApplicant.lastApplicantJobStep?.status ==
                              'ACCEPTED'
                          "
                        >
                          <v-chip
                      ...
                      <v-chip
                        v-else
                        :color="getColorStatus(selectedApplicant.status)"
                        variant="tonal"
                      >
                        {{ capitalizeWords(selectedApplicant.status) }}
                      </v-chip>
                    </div>
                  </div>
                </v-list-item>
              ...
              <v-card-text class="pt-0">
                ...
                <div class="job-details">
                  ...
                  <div class="mb-6">
                    ...
                    <span
                      v-if="
                        !selectedJob.right_to_works ||
                        selectedJob.right_to_works.length === 0
                      "
                      >Not specified</span
                    >
                  </div>
            ...
            <v-card flat>
              <v-toolbar density="comfortable" class="bg-transparent px-2" flat>
                <v-row class="align-center px-3" style="min-width: 220px">
                  <v-btn
                    icon
                    size="small"
                    @click="router.replace('/admin/jobs/applied')"
                    elevation="0"
                    color="primary"
                    class="mr-2"
              ...
              <template v-if="selectedJob">
                ...
                <v-card-text class="px-4">
                  ...
                  <!-- Job Details -->
                  <div class="job-details" style="font-size: 14px">
                    <h3 class="text-title mb-3">Job Description</h3>
                    <rich-text-view v-model="selectedJob.description" />
...
const fetchJobs = async (page = 1) => {
  ...
  params.value.filters = {
    "job.title": searchInput.value,
    "job.description": searchInput.value,
    "job.company.company_name": searchInput.value,
    "job.region.name": filterData.value.city,
    "job.other_region": filterData.value.city,
    "job.domicile_status": filterData.value.domicileStatus,
    "job.employment_status": filterData.value.employmentStatus,
    orWhere: [
      "job.title",
      "job.description",
  ...
  if (filterData.value.interest.length > 0) {
    params.value.filters["job.interest.id"] = filterData.value.interest;
  }
  if (filterData.value.skills.length > 0) {
    params.value.filters["job.skill.id"] = filterData.value.skills;
  }
  if (filterData.value.rightToWork.length > 0) {
    params.value.filters["job.right_to_work.id"] = filterData.value.rightToWork;
  }

  params.value.expands =
    "job.company, job.region, job.salary_country, job.right_to_work.salary_country, lastApplicantJobStep.jobStep";
  params.value.sortBy = {
    updated_at: "desc",
  };
  ...
  jobs.value.forEach((applicant) => {
    if (applicant.skill_match !== undefined && applicant.skill_match !== null) {
      applicant.job.skill_match = applicant.skill_match;
    }
  });
  ...
  if (response.items.length > 0) {
    if (route.query.id) {
      selectedJob.value = jobs.value.find(
        (applicant) => applicant.job.id === route.query.id
      )?.job;
      selectedApplicant.value = response.items.find(
        (applicant) => applicant.job.id === route.query.id
      );
      detailDialog.value = true;
    } else {
      selectedJob.value = response.items[0]?.job;
      selectedApplicant.value = response.items[0];
    }
  } else {
...
const handleUnsave = async () => {
  ...
  try {
    const applicant = jobs.value.find((a) => a.job.id === selectedJob.value.id);
    await $apiFetch(
      `/applicants/${selectedJob.value.id}/${applicant.user_id}`,
      {
        method: "DELETE",
      }
    );
...
const handleApply = async () => {
  ...
  try {
    ...
    router.push(`/admin/jobs/applied`);
    // Show success message
  } catch (error) {
...
const fetchJobDetail = async (jobId) => {
  ...
  try {
    ...
    if (res.items && res.items.length > 0) {
      ...
      const applicantIndex = jobs.value.findIndex((a) => a.job.id === jobId);
      if (applicantIndex !== -1) {
        jobs.value[applicantIndex].job = {
          ...jobs.value[applicantIndex].job,
          ...jobDetail,
        };
      }
      selectedJob.value = jobDetail;
      selectedApplicant.value = jobs.value.find(
        (applicant) => applicant.job.id === jobId
      );
    }
  } catch (error) {
    console.error("Error fetching job detail:", error);
    // Fallback to existing job data
    const applicant = jobs.value.find((a) => a.job.id === jobId);
    if (applicant) {
      selectedJob.value = applicant.job;
...
const getColorStatus = (status) => {
  ...
  const statusColors = {
    ...
    PROCESS: "info", // Light blue - Sedang diproses
    SCHEDULE_INTERVIEW: "warning", // Orange - Jadwal interview
