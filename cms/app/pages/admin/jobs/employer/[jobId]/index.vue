<template>
  <v-container class="pb-0">
    ...
    <div class="d-flex align-center">
      ...
      <h3 v-if="selectedJob">{{ selectedJob?.title }} Applicants</h3>
    </div>
  </v-container>
  <v-container>
            ...
            <v-row class="mt-0">
              ...
              >
                <v-list class="my-1">
                  ...
                  >
                    <v-list-item
                      :to="`/admin/jobs/employer/${route.params.jobId}/${candidate.user_id}/review`"
                      class="py-4"
                      :class="{
                        'selected-candidate':
                    ...
                    >
                      <template v-slot:prepend>
                        <v-list-item-action class="mr-3">
                          ...
                          >
                            <v-img
                              v-if="candidate.user?.photo_url"
                              :src="
                                candidate.user.photo_url.startsWith('http')
                                  ? candidate.user.photo_url
                                  : BASE_URL + candidate.user.photo_url
                              "
                              cover
                            ></v-img>
                      ...
                      >
                        <div class="d-flex align-center gap-2 flex-wrap">
                          ...
                          <v-img
                            v-if="candidate.user?.is_skill_passport_verified"
                          ...
                          <v-avatar
                            color="brown"
                            size="20"
                            v-if="candidate.user?.is_school_verified"
                          >
                            <v-icon size="18">mdi-school-outline</v-icon>
                          </v-avatar>

                          <!-- Chip section for desktop - sejajar dengan nama -->
                          <template v-if="isDesktop">
                            <template v-if="candidate.lastApplicantJobStep">
                              <template
                                v-if="
                                  candidate.lastApplicantJobStep?.jobStep
                                    ?.type == 'SYS_HIRED' &&
                                  candidate.lastApplicantJobStep?.status ==
                                    'ACCEPTED'
                                "
                              >
                                <v-chip
                            ...
                            <v-chip
                              ...
                              size="small"
                              :color="
                                getColorStatus(candidate.status, 'employer')
                              "
                              variant="tonal"
                            >
                              {{ candidate.status }}
                            ...
                            <SkillMatchBadge
                              v-if="
                                candidate.skill_match !== undefined &&
                        ...
                        <!-- Chip section for mobile - baris baru -->
                        <div
                          v-if="isMobile"
                          class="d-flex gap-2 my-1 flex-wrap"
                        >
                          <template v-if="candidate.lastApplicantJobStep">
                            <template
                              v-if="
                                candidate.lastApplicantJobStep?.jobStep?.type ==
                                  'SYS_HIRED' &&
                                candidate.lastApplicantJobStep?.status ==
                                  'ACCEPTED'
                              "
                      ...
                      <div>
                        <div>
                          ...
                          >
                            <template
                              v-for="(
                                career, index
                              ) in candidate.user?.userCareerHistories
                        ...
                        <div>
                          <span
                            class="text-grey text-caption d-inline-block"
                            style="
                              white-space: nowrap;
                              overflow: hidden;
                              text-overflow: ellipsis;
                            ...
                            "
                            >{{
                              candidate.user?.is_outside_indo
                                ? candidate.user?.other_country
                                : candidate.user?.region
                                ? "Indonesia"
                                : "N/A"
                            }}
                          </span>
                        ...
                        >
                          <span
                            class="text-grey"
                            v-if="candidate.user?.birth_year"
                            >{{
                              getAge(candidate.user?.birth_year) || "N/A"
                            }}</span
                          >
                          <br />
                          <span class="text-grey">
                            Last updated on
                            {{ formatDateWithoutWeekday(candidate.updated_at) }}
                          </span>
                        </div>
                      </div>
...
const fetchCandidates = async (page = 1) => {
  ...
  params.value.filters = {
    ...
    "user.other_region": filterData.value.city,
    "user.region.full_name": filterData.value.city,
    orWhere: [
      "user.full_name",
      "user.email",
      "user.other_region",
  ...
  params.value.expands =
    "job, user.region, user.userInterests, user.userCareerHistories, lastApplicantJobStep.jobStep";
  ...
  if (candidates.value?.length) {
    selectedJob.value = candidates.value[0].job;
  } else {
    fetchJob();
  }
...
const fetchJob = async () => {
  const response = await $apiFetch(`/jobs/${route.params.jobId}`);
