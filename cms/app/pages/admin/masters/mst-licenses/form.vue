        ...
        <v-card-text class="pa-6">
          ...
          <v-row v-else>
            ...
            >
              ...
              <div v-else-if="field.name === 'certificate_level'">
                ...
                <v-select
                  ...
                  density="comfortable"
                  color="primary"
                  :loading="certificateLevelLoading"
...
// Certificate Level Data
const certificateLevels = ref([]);
const certificateLevelLoading = ref(false);

const fetchCertificateLevels = async () => {
  try {
    certificateLevelLoading.value = true;
    const response = await $apiFetch("/configs/key/certificate_level");
    if (response && response.value && Array.isArray(response.value)) {
      certificateLevels.value = response.value;
    }
  } catch (error) {
    console.error("Error fetching certificate levels:", error);
  } finally {
    certificateLevelLoading.value = false;
  }
};
