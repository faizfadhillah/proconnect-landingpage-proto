        ...
        >
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn
              ...
              color="primary"
              density="compact"
              :to="`/admin/masters/${entity}/form?id=${item.id}`"
              class="mr-1"
            ></v-btn>
            <v-btn
              icon="mdi-delete"
...
const loadFields = async () => {
  ...
  if (Array.isArray(fields.value)) {
    ...
    if (entity.value == "mst-licenses") {
      const issue_date_column_index = selectedColumns.value.findIndex(
        (column) => column == "issue_date"
      );
      if (issue_date_column_index > -1) {
        selectedColumns.value.splice(issue_date_column_index, 1);
      }
    }
